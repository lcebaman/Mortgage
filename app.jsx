...
window.App = (() => {
  ...
  function MortgageCalculator() {
    ...
    const recalc = () => {
      const equityFromSale = Math.max(
        0,
        Number(currentPropertyValue) - Number(outstandingMortgage)
      );
      ...
      const loanAmount = Math.max(
        0,
        (Number(propertyPrice) || 0) - depositAvailable
      );
    ...
    const loanToValue =
      results.loanAmount > 0
        ? ((results.loanAmount / Math.max(1, propertyPrice)) * 100).toFixed(1)
        : "0.0";
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-4 bg-white border rounded-xl p-4 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-end gap-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder="e.g. 900k_house_4.5%_25y"
              ...
              <div className="flex-1">
                <label className="block text-sm font-medium mb-1">
                  Saved configs {(sb && user) ? "(supabase)" : "(local)"}
                </label>
                <select
                  value={selectedCfg}
            ...
            {!hasCreds && (
              <p className="text-xs text-gray-500 mt-2">
                Using local storage. Set <code>SUPABASE_URL</code> &amp;{" "}
                <code>SUPABASE_ANON_KEY</code> in <code>supabase_config.js</code> to enable cloud save.
              </p>
            )}
            {hasCreds && !user && (
              <p className="text-xs text-gray-500 mt-2">
          ...
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 md:p-8">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-white">
                  Property Move Calculator
                </h1>
              </div>
              <p className="text-blue-100">Sale proceeds, Buying &amp; Selling costs</p>
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6 p-6 md:p-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium mb-2">
                        Current Property Value: {formatCurrency(currentPropertyValue)}
                      </label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">
                        Outstanding Mortgage: {formatCurrency(outstandingMortgage)}
                      </label>
                      <input
                        type="number"
                      ...
                      <p className="text-xs text-gray-600 mt-1">
                        Your equity:{" "}
                        {formatCurrency(Math.max(0, currentPropertyValue - outstandingMortgage))}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 rounded-lg p-4 border-2 border-green-200">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div className="flex items-center gap-2 pt-6">
                        <input
                          ...
                          className="w-4 h-4"
                        />
                        <label htmlFor="ftb" className="text-sm font-medium">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">
                        Agency Fee (total) — includes Solicitor, Estate Agency, Legal, Buyer Agency,
                        Surveyor, EPC/Other
                      </label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">
                        Removal Costs: {formatCurrency(removalCosts)}
                      </label>
                      <input
                        type="number"
                        value={removalCosts}
                    ...
                    <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                      <div className="flex justify-between text-sm">
                        <span>LBTT (auto)</span>
                        <span className="font-semibold">{formatCurrency(results.lbtt)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-sm">
                        <span>Removals</span>
                        <span className="font-semibold">{formatCurrency(removalCosts)}</span>
                      </div>
                      <div className="flex justify-between font-bold mt-2 pt-2 border-t border-emerald-200">
                        ...
                        <span className="text-emerald-700">
                          {formatCurrency(results.totalTransactionCosts)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between pb-2 border-b border-emerald-300">
                      ...
                      <span className="font-bold">
                        {formatCurrency(results.totalLiquidFunds)}
                      </span>
                    </div>
                    <div className="flex justify-between pb-2 border-b border-emerald-300">
                      <span>Less: Buying Costs (LBTT + Agency Fee + Removals)</span>
                      <span className="font-semibold">
                        -{formatCurrency(results.buyingCosts)}
                      </span>
                    </div>
                    <div className="flex justify-between pt-2 text-base">
                      <span className="font-bold">Available Deposit</span>
                      <span className="font-bold text-lg">
                        {formatCurrency(Math.max(0, results.depositAvailable))}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* ✅ Green vs Red notice based on loan amount */}
                {Number(results.loanAmount) < 550000 ? (
                  <div className="bg-green-50 rounded-lg p-6 border-2 border-green-200">
                    ...
                    {results.totalLiquidFunds - results.buyingCosts - results.loanAmount > 0 && (
                      <p className="text-sm text-gray-600">
                        Remaining after deposit &amp; costs:{" "}
                        <span className="font-bold text-green-600">
                          {formatCurrency(
                            results.totalLiquidFunds - results.buyingCosts - results.loanAmount
                          )}
                        </span>
                      </p>
                    )}
                  ...
                  <div className="bg-red-50 rounded-lg p-6 border-2 border-red-300">
                    ...
                    <p className="text-red-800">
                      Your mortgage loan is <strong>{formatCurrency(results.loanAmount)}</strong>,
                      which may lead to high monthly payments.
                    </p>
          ...
          <div className="mt-6 text-center text-gray-600 text-sm">
            <p>
              This calculator is for illustration purposes only. Consult with a financial advisor for
              personalised advice.
            </p>
          </div>
        </div>
